...
def main():
    ...
    # Begin Game!
    ## PRO TIP: MULTIPLE BOARDS CAN WIN AT THE SAME TIME, FILTER THOSE OUT
    current_num = 0
    ...
    while len(calls) > 0:
        ...
        if winners:
            for win in winners:
                print(f"\nBingo!")
                print(boards[win])
                print(f"Summation: {boards[win].summation()}")
                print(f"Current Number: {current_num}")
                print(f"Final Score: {boards[win].summation() * current_num}")
